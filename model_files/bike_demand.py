...
# Reading the dataset 
dataset = pd.read_csv(r'C:\Users\olutu\ML_BIKE_DEMAND\model_files\SeoulBikeDemand.csv', encoding='unicode_escape')
...
dataset['Working Day'] = dataset['Working Day'].map({'Yes': 1, 'No': 0}).astype(int)
dataset['Holiday'] = dataset['Holiday'].map({'No Holiday': 0, 'Holiday': 1}).astype(int)
dataset['Seasons'] = dataset['Seasons'].map({'Winter': 1, 'Spring': 2, 'Summer': 3, 'Autumn': 4}).astype(int)
dataset.head()

# Separating the variables into X and y, independent and dependent variables 
X = dataset[['Time of Day', 'Temperature', 'Humidity', 'Wind Speed',
